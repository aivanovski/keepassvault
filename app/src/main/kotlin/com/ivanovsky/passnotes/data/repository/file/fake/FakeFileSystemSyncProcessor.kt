...
class FakeFileSystemSyncProcessor(
    ...
    private val syncResolutionResolver = SyncResolutionResolver()
    ...
    override fun getMergeFiles(uid: String): OperationResult<MergeFiles> =
        either {
            if (uid !in FileUid.CONFLICT_IDS) {
    ...
    override fun process(
        ...
        val baseFile = storage.getBaseFileOrNull(file.uid)
        val remoteFile = storage.getRemoteFileOrNull(file.uid)

        val resolution = syncResolutionResolver.resolve(
            isLocalModified = localFile?.modified.isNewerThan(baseFile?.modified),
            isRemoteModified = remoteFile?.modified.isNewerThan(baseFile?.modified),
            requestedResolution = requestedResolution
        )

        Timber.d(
            "process: resolution=%s, requestedResolution=%s, file.uid=%s",
            resolution,
            requestedResolution,
            file.uid
        )
        ...
        return when {
            resolution == SyncResolution.DOWNLOAD_REMOTE -> {
                downloadRemoteFile(file)
            }

            resolution == SyncResolution.UPLOAD_LOCAL -> {
                uploadLocalFile(file)
            }

            resolution == SyncResolution.NO_CHANGES && remoteFile != null ->
                OperationResult.success(remoteFile)
    ...
    private fun uploadLocalFile(file: FileDescriptor): OperationResult<FileDescriptor> {
        ...
        if (file.uid == FileUid.UPLOAD_FAILURE) {
            uidToSyncProgressStatusMap[file.uid] = SyncProgressStatus.UPLOADING
            notifySyncProgressChanges(file.uid, SyncProgressStatus.UPLOADING)
            return OperationResult.error(
                newGenericIOError(
                    "Unable to upload file: ${file.uid}",
                    Stacktrace()
                )
            )
        }
