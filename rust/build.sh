...
readonly CRATE_DIR="$SCRIPT_DIR/passnotes-rust"
readonly LOCAL_PROPERTIES_PATH="$REPO_ROOT/local.properties"
readonly OUTPUT_ROOT="$SCRIPT_DIR/android-libs"
readonly JNI_LIBS_DIR="$REPO_ROOT/app/src/main/jniLibs"
readonly LIBRARY_NAME="libpassnotes_rust.so"
readonly ANDROID_API_LEVEL="26"
readonly ABIS=("armeabi-v7a" "arm64-v8a" "x86_64")

build_mode="release"
copy_to_android_project="false"

usage() {
    cat <<'EOF'
Usage: ./rust/build.sh [--debug] [--copy]
...
Options:
  ...
  --copy    Copy built libraries into app/src/main/jniLibs
...
copy_to_android() {
    ...
    echo "Copying JNI libraries to $JNI_LIBS_DIR"
    for abi in "${ABIS[@]}"; do
        local source_library="$output_dir/$abi/$LIBRARY_NAME"
        local target_dir="$JNI_LIBS_DIR/$abi"

        mkdir -p "$target_dir"
        cp "$source_library" "$target_dir/$LIBRARY_NAME"
    done
}
...
output_dir="$OUTPUT_ROOT/$build_mode"
...
if [[ "$copy_to_android_project" == "true" ]]; then
